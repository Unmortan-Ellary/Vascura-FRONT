...
### Tools:
- [Vascura - Card Inspector](https://github.com/Unmortan-Ellary/Vascura-FRONT/blob/main/Tools/Vascura%20-%20Card%20Inspector%20v0.405.html): A Reader \ Converter Tool for Character Cards (V1-V2-V3 Chub.AI etc). Vascura FRONT can imitate work of `SillyTavern` or `Serene Pub` using its Lorebook System and Marco Engine (for single card), but first CHARA Card need to be converted into the right format (Lorebook Card) - this Tool do exactly that. You can inspect any CHARA card and EXPORT it for Vascura FRONT with different options. Also supports native Lorebook JSONs from Vascura FRONT.
...
7. **Samplers:**
   - **`{{kwarg KWARG_NAME VALUE}}`**: `{{kwarg enable_thinking false}}` - Adds `"chat_template_kwargs": {"enable_thinking": false}` argument for next message, KWARGs usully used to disable `thinking` of the model or any other parameter. each single `{{kwarg}}` parameter should have its own macro line, if same `{{kwarg}}` parameter repeated multiple times in the same request, last one will be used.
