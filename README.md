...
Each `USER Message` stores a `Snapshot` state of VAR variables, flags, texts that were `{{set}}` at the time of this USER Message was created. This allows to regenerate any LLM Message / form new USER Message using exact state of VAR Array that was actual on this `USER Message` creation. If you have a complex RPG system full of variables with flags, stats, texts etc, each `USER Message` will work as a save point. In short every `USER Message` on creation saves all current VAR Variables that `{{set}}` and `{{get}}` macros use as unique array snapshot, new LLM Message use snapshot from `USER Message` above it for generation.
