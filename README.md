...
Available when using LM Studio or Llama.Cpp / Kobold.Cpp as backend, this feature provides short suggestions for the LLM’s next response or for continuing your current text edit. Accept any suggestion instantly by pressing `Tab`.
...
Each `USER Message` stores a unique `Snapshot` state of VAR variables, flags, texts etc. This allows to regenerate any LLM Message / form new `USER Message` using exact state of VAR Array that was actual on this `USER Message` creation. If you have a complex RPG system full of variables with flags, stats, texts etc, each `USER Message` will work as a save point. In short every `USER Message` on creation saves all current VAR Variables that `{{set}}` and `{{get}}` macros use as unique array snapshot, new LLM Messages load and use snapshot from `USER Message` above it for generation, forming a chain history of unique `Snapshots` for VAR Array that can be reused.

### Tools:
- [Vascura CARD Inspector](https://github.com/Unmortan-Ellary/Vascura-FRONT/blob/main/Tools/Vascura%20CARD%20Inspector%20v0.413.html): A Reader \ Converter Tool for Character Cards (V1-V2-V3 Chub.AI etc). Vascura FRONT can roughly imitate work of `SillyTavern` or `Serene Pub` using its Lorebook System and Macro Engine (for single card only), but first CHARA Card needs to be converted into the right format (Vascura FRONT Card) - this Tool do exactly that. You can inspect any CHARA card and EXPORT it for Vascura FRONT with different options. Also supports native Lorebook JSONs from Vascura FRONT.
